...
<script type="text/javascript">
  ...
  function delete_tpl(tpl_name,id){
    if(confirm('确定删除模板“'+tpl_name+'”吗？')){
      $.post("<?php echo U('Advter/delete');?>",{id:id,table:'template'},function(ret){
        alert(ret.info);
        if(ret.status == 1){
  ...
  function advterTplEdit(id,obj){
    ...
    $('.spinner').show();
    if(!id) return false;
    $.get("<?php echo U('Advter/edit');?>",{id:id,table:'template',tpl:'advterTplEdit'},function(ret){
  ...
  BUI.use('common/search',function (Search) {
    ...
    // var enumObj = {"1":"禁用","0":"正常"},
      ...
      store = Search.createStore('<?php echo U("Advter/advterTplList");?>?table=template',{
        proxy : {
    ...
    function addFunction(){
      ...
      $('.mark').show();
      $('.spinner').show();
      $.get('<?php echo U("Advter/add");?>',{table:'template',tpl:'advterTplAdd'},function(ret){
        $('#content').html(ret._html);
        $('#content').show();
        $('.mark').hide();
        $('.spinner').hide();
