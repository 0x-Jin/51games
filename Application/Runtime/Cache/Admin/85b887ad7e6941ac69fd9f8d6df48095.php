...
<script type="text/javascript">
  ...
  function slideEdit(id){
    ...
    $('.spinner').show();
    if(!id) return false;
    $.get("<?php echo U('Website/slideEdit');?>",{id:id},function(ret){
  ...
  function slideDelete(id){
    ...
    if(!id) return false;
    if(confirm('确定删除该数据吗？')){
        $.post("<?php echo U('Website/slideDelete');?>",{id:id},function(ret){
        alert(ret.info);
        if(ret.status == 1){
  ...
  BUI.use('common/search',function (Search) {
    ...
    // var enumObj = {"1":"禁用","0":"正常"},
      ...
      store = Search.createStore('<?php echo U("Website/slideList");?>',{
        proxy : {
    ...
    function addFunction(){
      ...
      $('.mark').show();
      $('.spinner').show();
      $.get('<?php echo U("Website/slideAdd");?>','',function(ret){
        $('#content').html(ret._html);
        $('#content').show();
        $('.mark').hide();
        $('.spinner').hide();
