      ...
      <form id="searchForm" class="form-horizontal span48">
        ...
        <div class="row">
          ...
          <div class="control-group span8">
            ...
            <div class="controls">
              <input type="text" class="control-text" name="domain">
            </div>
          </div>
...
<script type="text/javascript">
  ...
  function cdnEdit(id){
    ...
    $('.spinner').show();   
    if(!id) return false;
    $.get("<?php echo U('Advter/edit');?>",{tpl:'cdnEdit',id:id,table:"<?php echo I('table');?>"},function(ret){
  ...
  BUI.use('common/search',function (Search) {
    ...
    // var enumObj = {"1":"禁用","0":"正常"},
      ...
      columns = [
          {title:'域名ID',dataIndex:'id',width:100,elCls:'center'},
          {title:'域名',dataIndex:'domain',width:300,elCls:'center'},
          {title:'服务器',dataIndex:'type',width:150,elCls:'center'},
          {title:'状态',dataIndex:'status',width:150,elCls:'center'},
          {title:'操作',dataIndex:'opt',width:100,elCls:'center'}
        ],
      store = Search.createStore('<?php echo U("Advter/cdnList");?>',{
        proxy : {
      ...
      gridCfg = Search.createGridCfg(columns,{
        tbar : {
          items : [
            {text : '<i class="icon-plus"></i>新增域名',btnCls : 'button button-small opt-btn',handler:addFunction}
          ]
        },
    ...
    function addFunction(){
      ...
      $('.mark').show();
      $('.spinner').show();
      $.get('<?php echo U("Advter/add");?>',{tpl:'cdnAdd',table:"<?php echo I('table');?>"},function(ret){
        $('#content').html(ret._html);
        $('#content').show();
        $('.mark').hide();
        $('.spinner').hide();
