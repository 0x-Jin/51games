      ...
      <form id="searchForm" class="form-horizontal span48">
        ...
        <div class="row">
          <div class="control-group span8">
            <label class="control-label">素材分类ID：</label>
            <div class="controls">
              <input type="text" class="control-text" name="material_type_id">
            </div>
          </div>
          ...
          <div class="control-group span8">
            ...
            <div class="controls">
              <input type="text" class="control-text" name="mtype_name">
            </div>
          </div>
...
<script type="text/javascript">
  function set_status(status,id,obj){
    if(status == 1){
      //关闭
      $.post("<?php echo U('Advter/materialTypeEdit');?>",{status:0,id:id},function(ret){
        if(ret.status == 1){
      ...
      //开启
      $.post("<?php echo U('Advter/materialTypeEdit');?>",{status:1,id:id},function(ret){
        if(ret.status == 1){
  ...
  //编辑
  function materialTypeEdit(id,obj){
    $('.bui-dialog').remove();
    $('.mark').show();
    $('.spinner').show();   
    if(!id) return false;
    var manager_id = $(obj).attr('manager_id');
    $.get("<?php echo U('Advter/materialTypeEdit');?>",{manager_id:manager_id,id:id},function(ret){
  ...
  BUI.use('common/search',function (Search) {
    ...
    // var enumObj = {"1":"禁用","0":"正常"},
      ...
      columns = [
          {title:'素材ID',dataIndex:'material_type_id',width:100,elCls:'center'},
          {title:'素材分类名称',dataIndex:'mtype_name',width:100,elCls:'center'},
          {title:'状态',dataIndex:'status',width:100,elCls:'center',/*renderer:BUI.Grid.Format.enumRenderer(enumObj),*/renderer : function (value,obj) {
              if(value == 1){
                //正常
                return '<img data-tdtype="toggle" data-field="status" class="status_btn" onclick="set_status('+value+','+obj.material_type_id+',this)" data_status='+value+' data_id='+obj.id+' src="/static/admin/img/toggle_enabled.gif">';
              }else if(value == 0){
                //禁止
                return '<img data-tdtype="toggle" data-field="status" class="status_btn" onclick="set_status('+value+','+obj.material_type_id+',this)" data_status='+value+' data_id='+obj.id+' src="/static/admin/img/toggle_disabled.gif">';
              }
            }
          },
          {title:'创建时间',dataIndex:'real',width:100,elCls:'center'},
          {title:'操作',dataIndex:'opt',width:300,elCls:'center'}
        ],
      store = Search.createStore('<?php echo U("Advter/materialType");?>',{
        proxy : {
      ...
      gridCfg = Search.createGridCfg(columns,{
        tbar : {
          items : [
            {text : '<i class="icon-plus"></i>新增素材分类',btnCls : 'button button-small opt-btn',handler:addFunction}
          ]
        },
    ...
    function addFunction(){
      ...
      $('.mark').show();
      $('.spinner').show();
      $.get('<?php echo U("Advter/materialTypeAdd");?>','',function(ret){
        $('#content').html(ret._html);
        $('#content').show();
        $('.mark').hide();
        $('.spinner').hide();
