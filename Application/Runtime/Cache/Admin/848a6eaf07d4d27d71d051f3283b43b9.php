  ...
  <div class="container">
    <div class="row">
      <form id="searchForm" class="form-horizontal span48">
        <input type="hidden" value="proxy" name="table">
        <div class="row">
          ...
          <div class="control-group span8">
            ...
            <div class="controls">
              <input type="text" class="control-text" name="proxyName">
            </div>
          </div>
...
<script type="text/javascript">
  ...
  //编辑
  function proxyEdit(id,obj){
    $('.bui-dialog').remove();
    $('.mark').show();
    $('.spinner').show();
    if(!id) return false;
    $.get("<?php echo U('Advter/edit');?>",{table:'proxy',id:id,tpl:'proxyEdit'},function(ret){
  ...
  BUI.use('common/search',function (Search) {
    ...
    // var enumObj = {"1":"禁用","0":"正常"},
      ...
      columns = [
          {title:'代理商ID',dataIndex:'id',width:50,elCls:'center'},
          {title:'代理商名称',dataIndex:'proxyName',width:200,elCls:'center'},
          {title:'创建时间',dataIndex:'createTime',width:200,elCls:'center'},
          {title:'操作',dataIndex:'opt',width:300,elCls:'center'}
        ],
      store = Search.createStore('<?php echo U("Advter/proxyList");?>',{
        proxy : {
      ...
      gridCfg = Search.createGridCfg(columns,{
        tbar : {
          items : [
            {text : '<i class="icon-plus"></i>新增代理商',btnCls : 'button button-small opt-btn',handler:addFunction}
          ]
        },
    ...
    function addFunction(){
      ...
      $('.mark').show();
      $('.spinner').show();
      $.get('<?php echo U("Advter/add");?>',{table:'proxy',tpl:'proxyAdd'},function(ret){
        $('#content').html(ret._html);
        $('#content').show();
        $('.mark').hide();
        $('.spinner').hide();
