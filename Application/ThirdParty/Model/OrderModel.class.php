...
class OrderModel extends Model
{
    ...
    public function getOrderLimit($map, $start, $length)
    {
        ...
        if($export == 1){
            return M($this->tableName, C("DB_PREFIX_API"),'CySlave')->where($map)->order("id DESC")->select();
        }else{
            return M($this->tableName, C("DB_PREFIX_API"),'CySlave')->where($map)->order("id DESC")->limit($start, $length)->select();
        }
    }
     ...
     * @return mixed
     */
    public function getIncomeOrderLimit($map, $start, $length)
    {
        return M($this->tableName, C("DB_PREFIX_API"),'CySlave')->alias('a')->field('a.*,SUM(a.amount) AS amount,FROM_UNIXTIME(a.createTime,"%Y-%m-%d") AS payTime')->join('LEFT JOIN lg_agent b ON a.agent=b.agent')->where($map)->group('a.payType,a.type,a.agent,payTime')->select();
    }
    ...
    {
        return M($this->tableName, C("DB_PREFIX_API"),'CySlave')->where($map)->count();
    }
    ...
    {
        return M($this->tableName, C("DB_PREFIX_API"),'CySlave')->where($map)->sum('amount');
    }
