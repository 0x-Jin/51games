...
function excel_file_upload($dirname = ''){
        ...
        $upload->allowExts = array('xls','csv', 'xlsx');
        $upload->rootPath = './Uploads/';
...
function file_upload_all($dirname = ""){
    ...
    $upload = new \Think\Upload();
    $upload->maxSize  = 20*1024*1024 ;// 设置附件上传大小
    $upload->allowExts = array('xls', 'csv', 'xlsx', 'pdf', 'jpg', 'png', 'rar', 'zip');
    $upload->replace = true;
    $upload->saveName = "";
    $upload->subName = '';
    $upload->rootPath = './Uploads/';
 ...
 * @return array 数组列表
 */
function getDataList($table,$_key,$prifix='la_',$map=array()) 
{
    $mod = M($table,$prifix);
