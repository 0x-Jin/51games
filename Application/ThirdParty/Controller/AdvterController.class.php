...
class AdvterController extends BackendController
{
    ...
    private $secretKey='IrKcjeNuLckiU5UhhV6VwEJCuQmxzueD';
    private $secretId='AKIDseFd0JMUM5rUvA9AuSox87tTr68zJZ3K';
    ...
    public function principalList()
    {
        if(IS_POST){
            ...
            foreach ($list['list'] as $key=>$val){
                ...
                $list['list'][$key]['opt'] = '<a href="javascript:;" onclick="principalEdit('.$val['id'].',this)">编辑</a> | <a href="javascript:;" onclick="principalDelete('.$val['id'].',this)">删除</a>';
    ...
    public function agentList()
    {
        if(IS_POST){
            ...
            $advterStaus = array(0=>'激活报送',1=>'充值完成报送');
            foreach ($list['list'] as $key=>$val){
                $list['list'][$key]['opt'] = '<a href="javascript:;" onclick="agentEdit('.$val['id'].',this)">编辑</a> | <a href="javascript:;" onclick="agentDelete('.$val['id'].',this)">删除</a>';
                ...
                $list['list'][$key]['packageStatusName'] = ($val['id'] > 398 && $val['pid'] != 0 && $val['agentType'] == 0)? ($val['packageStatus'] == 2? $this->agentPath.$val["agent"].".apk": $val["packageStatus"]): '-1';
                $rows[] = $list['list'][$key];
            }
            $arr = array('rows'=>(empty($rows) ? array() : $rows),'results'=>$list['count']);
            exit(json_encode($arr));
    ...
    public function agentEdit()
    {
        if(IS_POST){
            ...
            $info['account'] = $advter_account_list[$info['advterAccountId']]['account'];
    ...
    public function agentAdd()
    {
        if(IS_POST){
            ...
            //查出负责人所属的部门
            $department = D('Admin')->commonQuery('principal',array('id'=>$data['principal_id']),0,1,'department');
            if($gameType == 1) unset($data['bundleId'], $data['appleId']);
            $data['updateTime']  = time();
    ...
    public function packageAgent()
    {
        if(IS_POST){
            ...
            if ($data["type"] == "1") {
                ...
                //判断渠道号是否符合标准
                if ($agent["id"] <= 398 || $agent["gameType"] != "1" || $agent["agentType"] != "0" || $agent["pid"] == "0" || $agent["packageStatus"] != "2") {
                    $this->error("该渠道号".$data["agent"]."无法进行重打包操作！");
                }
                //重打包
                if (D("Admin")->commonExecute("agent", array("agent" => $data["agent"], "id" => array("GT", 388), "gameType" => 1, "agentType" => 0, "pid" => array("NEQ", 0), "packageStatus" => 2), array("packageStatus" => 0, "updateTime" => time()), "lg_")) {
                    $this->success('操作成功');
                } else {
                    $this->error('操作失败');
                }
                ...
                foreach ($agent as $val) {
                    if ($val["id"] <= 398 || $val["gameType"] != "1" || $val["agentType"] != "0" || $val["pid"] == "0" || $val["packageStatus"] != "2") {
    ...
    public function channelTypeList()
    {
        if(IS_POST){
            ...
            foreach ($list['list'] as $key=>$val){
                $list['list'][$key]['opt'] = '<a href="javascript:;" onclick="channelTypeEdit('.$val['id'].',this)">编辑</a>';
                $list['list'][$key]['advteruserName'] = $advteruser_list[$val['advteruser_id']]['company_name'];
                $list['list'][$key]['principalName'] = $principal_list[$val['principal_id']]['principal_name'];
                $list['list'][$key]['createTime'] = date('Y-m-d H:i:s',$val['createTime']);
                $rows[] = $list['list'][$key];
    ...
    public function advListBatchAdd()
    {
        if(IS_POST){
            ...
            !$data['advlistId'] && $this->error('广告ID不能为空');
            $advNameArr = explode(',', $data['adv_name']);
            $agentArr   = explode(',', $data['agent']);
            if(count($advNameArr) != count($agentArr)) $this->error('广告名称和包编号个数必须对应');

            $adv_name = D('Admin')->commonQuery('advter_list',array('adv_name'=>array('IN',$advNameArr)));
            if(!empty($adv_name)) $this->error('存在相同的广告名称');

            //查出模板落地页的信息
            $advlistTpl = D('Admin')->commonQuery('advter_list',array('id'=>$data['advlistId']));
            if(!$advlistTpl) $this->error('广告模板ID不存在');
            $agentId = getDataList('agent','agent',C('DB_PREFIX_API'),array('game_id'=>$advlistTpl['game_id'],'agent'=>array('IN',$this->agentArr)));

            //是否全屏下载
            $android_url_down_arr = explode(',',$advlistTpl['android_url_down']);
            $ios_url_down_arr = explode(',',$advlistTpl['ios_url_down']);
            ...
            foreach ($advNameArr as $key => $value) {
                if(!isset($agentId[$agentArr[$key]])) continue;
                $android_url_down = array();
                $ios_url_down = array();
                //安卓链接
                foreach($android_url_down_arr as $k=>$v){
                    if(empty($v)){
                        $android_url_down[] = '';
                    }else{
                        $android_url_down[] = 'https://static.chuangyunet.net/'.$agentArr[$key].'.apk';
                    }
                }

                //IOS链接
                foreach($ios_url_down_arr as $k2=>$v2){
                    if(empty($v2)){
                        $ios_url_down[] = '';
                    }else{
                        $ios_url_down[] = $v2;
                ...
                //生成监控链接
                $params = D('Admin')->commonQuery('advteruser',array('id'=>$agentId[$agentArr[$key]]['advteruser_id']),0,1,'param,id');

                if(!empty($params['param'])){
                    if($params['id'] == 7){
                        //UC头条
                        if($agentId[$agentArr[$key]]['gameType'] == 1){
                            $data['monitor_link'] = "https://count.chuangyunet.net/AdvterClick.php?agent={$agent_info['agent']}&game_id={$agent_info['game_id']}&cy_os={$agent_info['gameType']}&adUserId={$params['id']}&muid={IMEI_SUM}&{$params['param']}";
                        }elseif($agentId[$agentArr[$key]]['gameType'] == 2){
                            $data['monitor_link'] = "https://count.chuangyunet.net/AdvterClick.php?agent={$agent_info['agent']}&game_id={$agent_info['game_id']}&cy_os={$agent_info['gameType']}&adUserId={$params['id']}&muid={IDFA_SUM}&{$params['param']}";
                        }
                        
                    }else{
                        $data['monitor_link'] = "https://count.chuangyunet.net/AdvterClick.php?agent={$agent_info['agent']}&game_id={$agent_info['game_id']}&cy_os={$agent_info['gameType']}&adUserId={$params['id']}&{$params['param']}";
                    }
                }else{
                    $monitor_link = '';
                }
                ...
                $param[] = array(
                    'adv_name'          => $value,
                    'adv_title'         => $advlistTpl['adv_title'],
                    'game_id'           => $advlistTpl['game_id'],
                    'agent_id'          => $agentId[$agentArr[$key]]['id'],
                    'adv_tpl_id'        => $advlistTpl['adv_tpl_id'],
                    'principal_id'      => $advlistTpl['principal_id'],
                    'html_filename'     => '',
                    'url_more'          => $advlistTpl['url_more'],
                    'android_url_down'  => $android_url_down,
                    'ios_url_down'      => $ios_url_down,
                    'material_id'       => $advlistTpl['material_id'],
                    'cnzz_code'         => $advlistTpl['cnzz_code'],
                    'advteruser_id'     => $agentId[$agentArr[$key]]['advteruser_id'],
                    'create_time'       => time(),
                    'copyright_company' => $advlistTpl['copyright_company'],
                    'company_address'   => $advlistTpl['company_address'],
                    'company_tel'       => $advlistTpl['company_tel'],
                    'auto_download'     => $advlistTpl['auto_download'],
                    'color'             => $advlistTpl['color'],
                    'bgcolor'           => $advlistTpl['bgcolor'],
                    'creater'           => session('admin.realname'),
                    'cdnId'             => $advlistTpl['cdnId'],
                    'isall'             => $advlistTpl['isall'],
                    'monitor_link'      => $monitor_link,
                    'status'            => $advlistTpl['status'],
                );
    ...
    public function advList()
    {
        if(IS_POST){
            ...
            // include('./static/admin/js/phpqrcode/phpqrcode.php');

            foreach ($list['list'] as $key=>$val){
                //生成二维码
                // @mkdir("Game/QR");
                // $outside    = "Game/QR/QR_ADV_".$val['id'].".png";
                // @unlink($outside);
                // $address    = $this->cdnDomains[$val['cdnId']].$val['html_filename'];
                // \QRcode::png($address, $outside, 'L', 5, 1);

                // <a target="_blank" href="'.U('Advter/preview',array('adv_id'=>$val['id'])).'">预览</a> | 
                $list['list'][$key]['monitor_link'] = htmlspecialchars($val['monitor_link']);
                $list['list'][$key]['opt'] = '<a target="_blank" href="'.U('Advter/preview',array('adv_id'=>$val['id'])).'">预览</a> | <a onclick=recreate('.$val['id'].','.$val['material_id'].','.$val['cdnId'].') href="javascript:;">重新生成</a> | <a onclick=advListEdit('.$val['id'].') href="javascript:;">编辑</a> | <a onclick=advListDelete('.$val['id'].') href="javascript:;">删除</a>';
    ...
    private function create_static_tpl_new($aid, $create_flag = false, $preview = false) {
        ...
        if ($preview || $create_flag || ($create_flag === false && ! file_exists($htmlfile))) { // 文件是否已经存在
            ...
            $this->assign('domain', 'https://fall.chuangyunet.net/'); // 素材相关资
            if (! $preview) {
                $this->assign('preview', false);
                $this->assign('path_url', 'https://fall.chuangyunet.net');
    ...
    public function advterTplList()
    {
        if(IS_POST){
            ...
            foreach ($list['list'] as $key=>$val){
                $list['list'][$key]['opt'] = '<a href="javascript:;" onclick="advterTplEdit('.$val['id'].',this)">编辑</a> | <a onclick=delete_tpl("'.$val['tpl_name'].'",'.$val['id'].') href="javascript:;">删除</a>';
                $rows[] = $list['list'][$key];
            }
            $arr = array('rows'=>(empty($rows) ? array() : $rows),'results'=>$list['count']);
            exit(json_encode($arr));
    ...
    public function proxyList()
    {
        if(IS_POST){
            ...
            foreach ($res['list'] as $key=>$val){
                $res['list'][$key]['opt'] = '<a href="javascript:;" onclick="proxyEdit('.$val['id'].',this)">编辑</a>';
                $rows[] = $res['list'][$key];
            }
    ...
    public function advterAccountList()
    {
        if(IS_POST){
            ...
            foreach ($res['list'] as $key=>$val){
                ...
                $res['list'][$key]['opt'] = '<a href="javascript:;" onclick="advterAccountEdit('.$val['id'].',this)">编辑</a>';
                $rows[] = $res['list'][$key];
            }
            $arr = array('rows'=>(empty($rows) ? array() : $rows),'results'=>$results);
            exit(json_encode($arr));
    ...
    public function advterUserList()
    {
        if(IS_POST){
            ...
            foreach ($res['list'] as $key=>$val){
                ...
                $res['list'][$key]['opt'] = '<a href="javascript:;" onclick="advterUserEdit('.$val['id'].',this)">编辑</a>';
                $rows[] = $res['list'][$key];
            }
            $arr = array('rows'=>(empty($rows) ? array() : $rows),'results'=>$results);
            exit(json_encode($arr));
    ...
    public function advterCompanyList()
    {
        if(IS_POST){
            ...
            foreach ($res['list'] as $key=>$val){
                $res['list'][$key]['create_time'] = date('Y-m-d H:i:s',$val['create_time']);
                $res['list'][$key]['opt'] = '<a href="javascript:;" onclick="advterCompanyEdit('.$val['id'].',this)">编辑</a>';
                $rows[] = $res['list'][$key];
            }
            $arr = array('rows'=>(empty($rows) ? array() : $rows),'results'=>$results);
            exit(json_encode($arr));
    ...
    public function _before_update($data)
    {
        if($this->table == 'advter_company_user'){
            ...
            $old_adv_name = D('Admin')->commonQuery('advteruser',array('id'=>$data['id']));
            $adv_name = D('Admin')->commonQuery('advteruser',array('company_name'=>$data['company_name']));
            if(($old_adv_name['company_name'] != $adv_name['company_name']) && !empty($adv_name['company_name'])){
            ...
            //旧的广告名
            $old_adv_name = D('Admin')->commonQuery('advter_list',array('id'=>$data['id']));
            //判断广告名称是否存在
            $adv_name = D('Admin')->commonQuery('advter_list',array('adv_name'=>$data['adv_name']));
            if(($old_adv_name['adv_name'] != $adv_name['adv_name']) && !empty($adv_name['adv_name'])){
            ...
            //生成监控链接
            $param = D('Admin')->commonQuery('advteruser',array('id'=>$agent_info['advteruser_id']),0,1,'param,id');

            if(!empty($param['param'])){
                if($param['id'] == 7){
                    //UC头条
                    if($agent_info['gameType'] == 1){
                        $data['monitor_link'] = "https://count.chuangyunet.net/AdvterClick.php?agent={$agent_info['agent']}&game_id={$agent_info['game_id']}&cy_os={$agent_info['gameType']}&adUserId={$param['id']}&muid={IMEI_SUM}&{$param['param']}";
                    }elseif($agent_info['gameType'] == 2){
                        $data['monitor_link'] = "https://count.chuangyunet.net/AdvterClick.php?agent={$agent_info['agent']}&game_id={$agent_info['game_id']}&cy_os={$agent_info['gameType']}&adUserId={$param['id']}&muid={IDFA_SUM}&{$param['param']}";
                    }
                    
                }else{
                    $data['monitor_link'] = "https://count.chuangyunet.net/AdvterClick.php?agent={$agent_info['agent']}&game_id={$agent_info['game_id']}&cy_os={$agent_info['gameType']}&adUserId={$param['id']}&{$param['param']}";
                }
            ...
            //判断是否存在同名负责人
            $old_name = D('Admin')->commonQuery('principal',array('id'=>$data['id']));
            ...
            if(($old_name['principal_rolename'] != $data['principal_rolename']) && !empty($principal_rolename['principal_rolename'])){
                IS_AJAX && $this->ajaxReturn(0, L('负责人简称已存在'));
                $this->error(L('负责人简称已存在'));
            }

            if(($old_name['principal_name'] != $data['principal_name']) && !empty($principal_name['principal_name'])){
    ...
    public function material()
    {
        ...
        $data['author'] && $map['author'] = D('Admin')->commonQuery('admin', array(
            'real' => $data['author']
        ))['id'];
        ...
        $materialtype = getDataList('material_type','material_type_id','la_',array('status'=>1));
    ...
    public function materialType(){
        if(IS_POST){
            ...
            foreach ($res['list'] as $key=>$val){
                $res['list'][$key]['create_time'] = date('Y-m-d H:i:s',$val['create_time']);
                $res['list'][$key]['opt'] = '<a href="javascript:;" onclick="materialTypeEdit('.$val['material_type_id'].',this)">编辑</a>';
                $rows[] = $res['list'][$key];
            }
