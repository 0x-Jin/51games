...
class AjaxController extends BackendController
{
    ...
    public function getAdvterAccount()
    {
        ...
        $all                                            = (int) $_REQUEST['all'];
        $map                                            = array();
        $search["advteruserId"] && $map["advteruserId"] = $search["advteruserId"];
        $search["proxyId"] && $map["proxyId"]           = $search["proxyId"];

        $list = M('advter_account', C('DB_PREFIX'), 'CySlave')->where($map)->field('id,account')->select();
    ...
    public function agent()
    {
        ...
        $where = 'status=0';
        $packageStatus && $where .= ' AND packageStatus = 2';
        $game_id && $where .= ' AND game_id = ' . $game_id;
        $aid && $where .= ' AND advteruser_id=' . $aid;
        $partmentId && $where .= ' AND departmentId=' . $partmentId;
        $data = M('agent', C('DB_PREFIX_API'), 'CySlave')->field("id,CONCAT(agentName,'[',agent,']') AS agent")->where($where)->select();
    ...
    public function getAgentByGame()
    {
        ...
        if (false !== ($key = array_search("--请选择母包--", $agent)) || false !== ($key = array_search("--全部--", $agent))) {
            array_splice($agent, $key, 1);
        }

        if (false !== ($key = array_search("--全部--", $game_id))) {
            array_splice($game_id, $key, 1);
        }

        if (false !== ($key = array_search("--全部--", $advteruser_id))) {
            array_splice($advteruser_id, $key, 1);
        }
        ...
        if ($agent) {
            $pid = array_keys(getDataList('agent', 'id', C('DB_PREFIX_API'), array('agent' => array('IN', $agent))));
            if ($pid) {
    ...
    public function getChildAgentByAgent()
    {
        ...
        if (false !== ($key = array_search("--请选择母包--", $agent))) {
            array_splice($agent, $key, 1);
        }
    ...
    public function getGameList()
    {
        ...
        $all                                  = (int) $_REQUEST['all'];
        if (!in_array(session('admin.role_id'), array(1, 3, 27)) && session('admin.uid') != 85) {
            $map['_string'] = ' id <> 104';
        }

        if (session('admin.role_id') == '27') {
            $map['id'] = 104;
        }
        $field = 'id,gameName';
    ...
    public function getServerList()
    {
        ...
        $list                       = $mod->field("serverId,serverName,openTime")->where($map)->group("serverId")->order('id DESC')->select();
    ...
    public function getAgent()
    {
        ...
        if (is_array($game_id) && in_array('--全部--', $game_id)) {
            unset($game_id[array_search("--全部--", $game_id)]);
        }
        ...
        !empty($gameType) && $map['gameType']  = $gameType;
        !empty($depId) && $map['departmentId'] = $depId;
    ...
    public function getAgentCreater()
    {
        ...
        if (!S($cacheName)) {
            ...
            if ($all) {
                //ios渠道包的创建人
                $map['creater'] = array('NEQ','system');
                $iosList   = array_column(M('events', C('DB_PREFIX'), 'CySlave')->field('distinct creater as creater')->where($map)->select(), 'creater');
                $list      = array_column($list, 'creater');
                $insetList = array_merge($iosList, $list);
                foreach ($insetList as $key => $value) {
                    $newList[] = array('creater' => $value);
                }
                $list = $newList;
            }
    ...
    public function getEventByGroup()
    {
        ...
        $game_id        = I('game_id');
        $advteruser_id  = I('advteruser_id');
        $agent_p        = I('agent_p');
        if (false !== ($key = array_search("--全部--", $events_groupId))) {
            array_splice($events_groupId, $key, 1);
        }
        ...
        if (false !== ($key = array_search("--全部--", $agent_p))) {
            array_splice($agent_p, $key, 1);
        }
    ...
    public function getAgentLink()
    {
        ...
        $link = '';
        if (!$data['agent_id']) {
            exit(json_encode(array('link' => '', 'gameType' => '', 'creater' => '')));
        }
    ...
    public function getWebsiteHome()
    {
        ...
        $data['all'] && array_unshift($list, array('id' => 0, 'name' => '全部'));
        echo json_encode($list ? $list : array());
        exit();
    }
    ...
    public function getFallByGame()
    {
        ...
        $agent_id = array_keys(getDataList('agent', 'id', 'lg_', $map));
        ...
        if (empty($agent_id)) {
            ...
            $agent_id = array_merge($agent_id, $agent);
        }

        //查出对应的落地页
        $list = M('advter_list', C('DB_PREFIX'), 'CySlave')->where(array('agent_id' => array('IN', $agent_id)))->field("id,adv_name")->select();
     ...
     * @return [type] [description]
     */
    public function getCostGame()
    {
        $list = M('advter_cost')->field('distinct trim(gameName) as gameName')->select();
     ...
     * 清除S缓存
     * @AuthorHTL
     * @DateTime  2018-05-14T14:15:44+0800
     * @return    [type]                   [description]
     */
