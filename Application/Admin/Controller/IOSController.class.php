...
class IOSController extends BackendController
{
    ...
    public function event_add()
    {
        if(IS_POST){
            ...
            $sign_key      = '';
            $encrypt_key   = '';
            if(in_array($data['advteruser_id'],array(10,2,14,76))){
            ...
            for ($i=1; $i <= $data['num']; $i++) { 
                $events_name = $data['events_name'].'-'.$i;
                if(isset($nowEvents[$events_name])){
                    continue;
                }
                $updateEvent[] = $events_name;
                $insert[] = array(
                    'createTime'        => $time,
	...
	public function eventsStop()
	{
		...
		if($res){
            ...
            $url = "http://apisdk.chuangyunet.net/Api/Ajax/advterLock?sign=signCheck1233Cy&status=".$status.'&fileName='.$fileName;
            proxy_curl($url,$proxy);
            proxy_curl($url,$proxy2);
	...
	public function fallEvents()
	{
	    if(IS_POST){
	        ...
	        foreach ($list['list'] as $key => $val){
	            ...
	            $list['list'][$key]['gameName'] = $gameName[$val['agent']]['agentName'];
	            $list['list'][$key]['opt'] = createBtn('<a href="javascript:;" class="button button-primary" onclick="eventsEdit(\''.$val['id'].'\',this)">编辑</a>&nbsp;&nbsp;&nbsp;');
	            $list['list'][$key]['opt'] .= createBtn(' | &nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="eventsStop(\''.$val['id'].'\','.$val['status'].')">'.$status[$val['status']].'</a>');
	        }
	        $arr = array('rows' =>($list['list'] ? $list['list'] : array()), 'results' => $count);
	        exit(json_encode($arr));
	    }else{
	        if(IS_AJAX){
	            $respose = $this->fetch();
	...
	public function fallEventsAdd()
	{
	    if(IS_POST){
            ...
            $nowEvents = getDataList('events', 'events_name', C('DB_PREFIX')); // 现有推广活动名称
            if (D('Admin')->commonQuery('events', array(
	        ...
	        if($res){
	            $update = getDataList('events','id',C('DB_PREFIX'),array('events_name'=>array('IN',$updateEvent)));
	            foreach ($update as $key => $value) {
	                $link = $this->createFullLink($value['id'],$value['agent'],$value['game_id'],$value['advteruser_id'],$value['apple_link']);
	                //更新监控链接
	                D('Admin')->commonExecute('events',array('id'=>$value['id']),array('monitor_link'=>$link));
	            }
	            $this->success('操作成功');
	 ...
	 * 落地页监控链接
	 */
	protected  function createFullLink($advter_id,$agent,$game_id,$adid,$apple_link)
	{
	    $url = 'https://count.cmgcwl.cn/AdvIndex.php?type=2&events='.$advter_id.'&gf='.$agent.'_'.$game_id.'&adUser='.$adid.'&appleLink='.urlencode($apple_link);
	    $shortLink = curl_get('http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long='.urlencode($url));
    ...
    protected function createLink($advter_id,$agent,$game_id,$adid)
    {
        ...
        if(!empty($param['iosParam']) && $adid != 10){
            ...
            $link = "https://count.cmgcwl.cn/AdvterClickIos.php?events={$advter_id}&gf=".$agent.'_'.$game_id."&adUser={$param['id']}&{$param['iosParam']}";
        }elseif($adid == 10 || $adid == 2){
            $link = "https://count.cmgcwl.cn/AdvterClickIos.php?events={$advter_id}&gf=".$agent.'_'.$game_id."&adUser={$param['id']}";
        }
        return $link;
    }
    ...
    public function _before_update($data)
    {
        ...
        if($this->table == 'events' && $data['eventType'] != 2){
            if(I('config') == 1){
                return $data;
            }
        	if(!$data['events_name']){
        ...
        if ($this->table == 'events' && $data['eventType'] == 2) {
            ...
            $link = $this->createFullLink($data['id'],$data['agent'],$data['game_id'],$data['advteruser_id'],$data['apple_link']);
            $data['monitor_link'] 	= $link;
        }
        ...
        if($this->table == 'events_group'){
            ...
            if($gconfig == 'groupConfig'){
                ...
                if($group_config){
                    ...
                    D('Admin')->commonExecute('events',array('events_groupId'=>$id,'advteruser_id'=>array('IN',array(10,2,14,76))),$update);
                }
            }
