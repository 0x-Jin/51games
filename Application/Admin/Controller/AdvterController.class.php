...
class AdvterController extends BackendController
{
    ...
    //广告商配置参数
    private $sdkApiAdvter = array(
        //广点通
        2 => array("UserActionSetId", "AppSecretKey", "AccountId"),
        //今日头条
        6 => array("AppName", "Channel", "Aid", "Akey"),
    );
    ...
    private $bysToken  = '25cf184a5459322380100d24edd6900d';
    ...
    public function _initialize()
    {
        ...
        !S('cdn') && S('cdn', getDataList('cdn', 'id', C('DB_PREFIX'), array('status' => 1)));
        !S('alicdn') && S('alicdn', getDataList('cdn', 'id', C('DB_PREFIX'), array('status' => 1, 'type' => 2)));
        ...
        if (!empty($cdnDomains)) {
            foreach ($cdnDomains as $key => $value) {
                $this->cdnDomains[$key] = $value['domain'];
            }
        }
        if (!empty($aliUrl)) {
            $this->aliUrl = array_keys($aliUrl);
        }
    ...
    public function agentList()
    {
        if (IS_POST) {
            $data = I();
            $map  = array();
            // $data['agentName'] && $map['agentName'] = array('like','%'.$data['agentName'].'%');
            if ($data['agentTpl'] == '0') {
                $map['agentType'] = 1; //母包
            ...
            if (!in_array(session('admin.role_id'), array(1, 3))) {
                ...
                $map['agent']   = array('NOT IN', array('jyQIHOO', 'jyBAIDU', 'jyDUOYOU', 'jyLEYOU'));
            }
            ...
            if ($data['agent']) {
                if (!in_array(session('admin.role_id'), array(1, 3)) && in_array($data['agent'], array('jyQIHOO', 'jyBAIDU', 'jyDUOYOU', 'jyLEYOU'))) {
                    $map['agent'] = '-1';
                } else {
            ...
            $partmentType        = array(1 => '<span style="color:green;">发行一部</span>', 2 => '<span style="color:#ee5f5b;">发行二部</span>', 3 => '公共部门');
            ...
            foreach ($list['list'] as $key => $val) {
                ...
                //微信，广点通，百度信息流显示配置按钮
                if (in_array($val['advteruser_id'], array(10, 2, 14, 76))) {
                    $btn = createBtn('<a href="javascript:;"  onclick="agentConfig(\'' . $val['agent'] . '\',this)">配置</a>');
                    session('admin.role_id') == 1 && $list['list'][$key]['opt'] .= '&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; ' . $btn;
                    $list['list'][$key]['opt2'] .= $btn;
                } else {
                    $list['list'][$key]['opt2'] .= createBtn('<span style="color:gray">无需配置</span>');
                }

                if (!$channel_list[$val['channel_id']]['goodsId']) {
                    $list['list'][$key]['opt'] .= ' <a href="javascript:;" onclick="goodsShow(\'' . $val['agent'] . '\',this)">商品</a>';
                }
    ...
    public function agentEdit()
    {
        if (IS_POST) {
            $data = I();
            if (empty($data['mainbody_id'])) {
                $this->error('请选择所属主体');
            }
            if ($data['gameType'] == 1) {
                if (!$data['and_bundleId']) $this->error('包名未填写');
                //安卓包名
                $data['bundleId'] = $data['and_bundleId'];
                unset($data['and_bundleId'], $data['appleId'], $data['trialVer']);
            } elseif ($data['gameType'] == 2) {
                if (!$data['bundleId']) $this->error('包名未填写');
                unset($data['and_bundleId']);
            }
    ...
    public function agentStatusEdit()
    {
        if (IS_POST) {
            ...
            $agent = D('Admin')->commonQuery('agent', array('id' => $data['id']), 0, 1, 'agent,creater', 'lg_');
            if (session('admin.realname') != $agent['creater'] && session('admin.role_id') != 1) {
                $this->error('操作失败，只能操作自己创建的渠道包');
            }
            if (D('Admin')->commonExecute('agent', array('id' => $data['id']), $data, 'lg_')) {
                $this->success('操作成功');
            } else {
    ...
    public function agentAdd()
    {
        if (IS_POST) {
            ...
            /* $agentName = D('Admin')->commonQuery('agent',array('agentName'=>$data['agentName']),0,1,'agentName','lg_');
            if($agentName){
            $this->error('包名称已经存在');
            }*/
            ...
            if ($data["channel_id"]) {
                ...
                if ($channel) {
                    $data["channelVer"] = $channel["channelVer"];
                }
            ...
            if ($data['gameType'] == 1) {
                ...
                //安卓包名
                $data['bundleId'] = $data['and_bundleId'];
                unset($data['and_bundleId'], $data['appleId']);
            } elseif ($data['gameType'] == 2) {
                if (!$data['bundleId']) $this->error('包名未填写');
                unset($data['and_bundleId']);
            }
            ...
            if ($data['advteruser_id'] == 2) {
                $data['changePackage'] = 1;
                //安卓广点通的包名需要加上渠道号
                if ($data['gameType'] == 1) $data['bundleId'] .= '.'.$data['agent'];
            }

            if ($id = D('Admin')->commonAdd('agent', $data, 'lg_')) {
                //查询新后台用户表是否存在自动新增游戏和母包的用户
                $this->setNewBackstageUserInfo($id);
                $this->success('操作成功');
            } else {
                $this->error('操作失败');
            }
     ...
     */
    private function setNewBackstageUserInfo($pid)
    {
        if (!$pid) {
            return false;
        }
        ...
        $agentInfo = D('Admin')->commonQuery('agent', array('id' => $pid), 0, 1, "*", "lg_",'');

        foreach ($user as $value) {
            $gameId = explode(',', $value['gameId']);
            $pAgent = explode(',', $value['pAgentId']);
            if (in_array('0', $pAgent) && in_array($agentInfo['game_id'], $gameId)) {
                //给用户加入对于的母包权限
                $pAgentId = trim($value['pAgentId'], ',') . ',' . $pid;
                if (!in_array($value['roleId'],[3,5]) || ($value['roleId'] == 3 && $value['partmentId'] == $agentInfo['departmentId'])) {
                    D('Admin')->commonExecute('admin_new', array('id' => $value['id']), array('pAgentId' => $pAgentId));
                }
            }
        }
    }
    ...
    public function batchAddAgent()
    {
        if (IS_POST) {
            ...
            if (empty($agent['mainbody_id'])) {
                $this->error('母包的所属主体为空，请先编辑母包主体再创建');
            }

            if (!$agent['bundleId']) {
                $this->error('母包的包名为空，请先编辑母包包名再创建');
            }
            ...
            for ($i = ($data['maxAgent'] + 1); $i <= ($data['maxAgent'] + $data['agentnum']); $i++) {
                $agentNum       = preg_replace('/\d/s', '', $agent['agent']) . str_pad($i, 3, '0', STR_PAD_LEFT);
                $childAgent[]   = array(
                    ...
                    'bundleId'         => $agent['bundleId'].($changePackage == 1? ".".$agentNum: ""),
    ...
    public function advListBatchAdd()
    {
        if (IS_POST) {
            ...
            foreach ($agentArr as $key => $value) {
                ...
                $param[$key]['departmentId']     = session('admin.partment');
    ...
    public function advList()
    {
        if (IS_POST) {
            ...
            $map     = $this->parId ? array('departmentId' => array('IN', $this->parId)) : array();
            $creater = M('advter_list')->field('distinct creater')->where($map)->select();
    ...
    private function create_static_tpl_new($aid, $create_flag = false, $preview = false)
    {
        ...
        if (in_array($info['cdnId'], $this->aliUrl)) {
            $htmlfile = 'AliGame/' . $info['game_id'] . '/' . $info['agent'] . '/' . $aid . '.html';
        } else {
            $htmlfile = 'Game/' . $info['game_id'] . '/' . $info['agent'] . '/' . $aid . '.html';
        }
        if ($preview || $create_flag || ($create_flag === false && !file_exists($htmlfile)) || in_array($info['cdnId'], $this->aliUrl)) {
            // 文件是否已经存在
            ...
            if (!empty($tarr)) {
                foreach ($tarr as $k => $v) {
                    if ($v['pop'] != 1) {
                        ...
                        $popArr[]  = $v;
                        $popLink[] = $info['android_url_down'][$k] ?: $info['ios_url_down'][$k];
                    }
                }
            }
            ...
            if (!$preview) {
                ...
                $this->assign('path_url', $this->cdnDomains[1]);
    ...
    {
        $adv_id = array_keys(getDataList('advter_list', 'id', C('DB_PREFIX'), array('id' => array('neq', 7337), 'adv_tpl_id' => 2))); //CDNID列表
        foreach ($adv_id as $v) {
            $res    = $this->create_static_tpl_new($v, true);
            $file[] = $res;
        }
        var_dump(count($file));die();
    }
    ...
    public function recreate($aid = '')
    {
        ...
        foreach ($adv_id as $key => $value) {
            //测试白山云
            $urls[] = $this->cdnDomains[$cdnId[$value]['cdnId']] . $file[$key];
            $domain               = $this->cdnDomains[$cdnId[$value]['cdnId']];
            $urlArr['urls.' . $n] = $domain . $file[$key];
            $n++;
            $urlArr['urls.' . $n] = $this->cdnDomains[1] . $file[$key];
            $n++;
        ...
        if (!empty($urls) && count($urls) > 0) {
            $params['urls'] = $urls;
            $params['type'] = 'url';
            $return_data    = json_decode(push_cdn($this->bysToken, $params), true);
            if (!empty($return_data)) {
                $bsyreturn_data = array('code' => 'success', 'data' => array('count' => count($return_data)));
            }
        }
        ...
        if ($bsyreturn_data && $return_data) {
            $return_data = array(
                'code' => 'bsyRes_'.$bsyreturn_data['code'].',txRes_'.$return_data['code'], 
                'data' => array(
                    'count' => 'bsy_'.$bsyreturn_data['data']['count'].','.'tx_'.$return_data['data']['count']
                )
            );
        }
...
//                $advteruserIdArr = explode(',', $val['advteruserId']);
    ...
    public function rebateEdit()
    {
        if (IS_POST) {
            $data = I();
            if (!in_array(session("admin.role_id"), array(17, 25))) $this->error("您无权进行该操作！");
            if (!$data["advteruserId"]) $this->error("请选择广告商！");
            if (!$data["proxyId"]) $this->error("请选择代理商！");
            $data["rebate"] && $data["rebate"] = str_replace("%", "", $data["rebate"]);
            $data["updateTime"] = date("Y-m-d H:i:s");
            if (D("Admin")->commonExecute("advter_account", array("advteruserId" => $data["advteruserId"], "proxyId" => $data["proxyId"]), $data)) {
                $this->success("修改返点成功！");
            } else {
                $this->error("修改返点失败！");
            }
            ...
            if (in_array(session("admin.role_id"), array(17, 25))) {
                ...
                $this->ajaxReturn(array("status" => 0, "_msg" => "您无权进行该操作！"));
            }
        }
    }
    ...
    public function _get_before_update($table, $info)
    {
        if ($table == 'advteruser') {
            ...
            $this->assign('partment', $this->partment);
        } elseif ($table == 'advter_account') {
            $admin_key = in_array(session("admin.role_id"), array(17, 25)) ? 1 : 0;
            $this->assign("key", $admin_key);
        }
    }
...
//            $proxyId      = I('request.proxy');
    ...
    {
        if ($adid == 2) {
            $link = "https://count.cmgcwl.cn/AdvterClick.php?agent={$agent}&game_id={$game_id}&cy_os=1&adUserId=2";
            return $link;
        }
        //生成监控链接
        ...
        if (!empty($param['param'])) {
            if ($param['id'] == 7) {
                //UC头条
                if ($os == 1) {
                    $link = "https://count.cmgcwl.cn/AdvterClick.php?agent={$agent}&game_id={$game_id}&cy_os={$os}&adUserId={$param['id']}&muid={IMEI_SUM}&{$param['param']}";
                }

            } else {
                if ($os == 1) {
                    $link = "https://count.cmgcwl.cn/AdvterClick.php?agent={$agent}&game_id={$game_id}&cy_os={$os}&adUserId={$param['id']}&{$param['param']}";
                }
            }
    ...
    public function copyMaterial()
    {
        ...
        if ($minfo && $finfo) {
            ...
            if ($lastId) {
                //复制图片 35_0.png"
                if (!file_exists('./Uploads/material/' . $lastId)) {
                    mkdir('./Uploads/material/' . $lastId, 0755, true);
                }
    ...
    public function agentConfig()
    {
        $data = I();
        if (IS_POST) {
            $agentInfo = D('Admin')->commonQuery('agent', array(
                'agent' => $data['agent'],
            ), 0, 1, '*', 'lg_');
            $data['game_id']       = $agentInfo['game_id'];
            $data['advteruser_id'] = $agentInfo['advteruser_id'];
            $data['department']    = $agentInfo['departmentId'];
            if ($agentInfo['advteruser_id'] != 2) {
                $data['config_conv_type'] = '';
            }
            if (!empty($data['agent']) && $data['status'] == 0) {
                $res = D('Admin')->commonExecute('and_agent_config', array('agent' => $data['agent']), $data, 'lg_');
            } elseif (!empty($data['agent']) && $data['status'] == 1) {
                $res = D('Admin')->commonAdd('and_agent_config', $data, $prefix = 'lg_');
            }
            ...
            $list = D('Admin')->commonQuery('and_agent_config', array(
                'agent' => $data['agent'],
            ), 0, 1, '*', 'lg_');
            ...
            $list['advteruser_id'] = $agentInfo['advteruser_id'];
            $this->assign('info', $list);
            if (IS_AJAX) {
                $response = $this->fetch();
                $this->ajaxReturn(array(
    ...
    public function adverParam()
    {
        if (IS_POST) {
            ...
            $res                            = D("Admin")->getBuiList("adver_param", $map, $start, $pageSize, C("DB_PREFIX_API"));
            $agentList                      = getDataList("agent", "agent", C("DB_PREFIX_API"));
            $advterList                     = getDataList("advteruser", "id");
            $info                           = array();
            foreach ($res["list"] as $val) {
                $row                   = $val;
                $row["agentName"]      = $agentList[$val["agent"]]["agentName"];
                $row["advteruserName"] = $advterList[$val["advteruser_id"]]["company_name"];
                $info[]                = $row;
            }
            $arr = array("rows" => $info, $res["count"]);
            exit(json_encode($arr));
        } else {
            $this->display();
        }
    ...
    public function paramAdd()
    {
        if (IS_POST) {
            $data = I();
            if (!$data["agent"] || !$data["advteruser_id"]) {
                $this->error("请填写渠道号并选择广告商");
            }

            if (!isset($this->sdkApiAdvter[$data["advteruser_id"]])) {
                $this->error("该广告商暂时无法配置");
            }

            $agent = D("Admin")->commonQuery("agent", array("agent" => $data["agent"]), 0, 1, "agent,gameType,advteruser_id,packageStatus,pid", C("DB_PREFIX_API"));
            if (!$agent) {
                $this->error("包体编号输入错误");
            }

            if ($agent["gameType"] == "1" && $agent["advteruser_id"] != $data["advteruser_id"]) {
                $this->error("该包体的广告商不匹配");
            }

            if ($agent["packageStatus"] != "2" && $agent["pid"]) {
                $this->error("渠道包必须打包完成才可以设置参数");
            }

            $param = D("Admin")->commonQuery("adver_param", array("agent" => $data["agent"], "advteruser_id" => $data["advteruser_id"]), 0, 1, "agent,advteruser_id", C("DB_PREFIX_API"));
            if ($param) {
                $this->error("包体编号的该广告商配置已存在");
            }

            $data["createTime"] = time();
            $data["status"]     = 0;
            foreach ($this->sdkApiAdvter[$data["advteruser_id"]] as $key => $value) {
                $data["param" . ($key + 1)] = $value;
            }
            if (D("Admin")->commonAdd("adver_param", $data, C("DB_PREFIX_API"))) {
                D("Admin")->commonExecute("agent", array("agent" => $data["agent"]), array("packageStatus" => 0), C("DB_PREFIX_API"));
                $this->success("添加配置成功");
            } else {
                $this->error("添加配置失败");
            };
    ...
    public function paramEdit()
    {
        if (IS_POST) {
            $data = I();
            if (!$data["id"]) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "获取不到ID！"));
            }

            $param = D("Admin")->commonQuery("adver_param", array("id" => $data["id"]), 0, 1, "*", C("DB_PREFIX_API"));
            if (!$param) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "获取不到参数配置"));
            }

            $agent = D("Admin")->commonQuery("agent", array("agent" => $param["agent"]), 0, 1, "agent,gameType,advteruser_id,packageStatus,pid", C("DB_PREFIX_API"));
            if (!$agent) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "包体编号输入错误"));
            }

            if ($agent["packageStatus"] != "2" && $agent["pid"]) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "渠道包必须打包完成才可以设置参数"));
            }

            $data["updateTime"] = time();
            if (D("Admin")->commonExecute("adver_param", array("id" => $data["id"]), $data, C("DB_PREFIX_API"))) {
                if ($data["change"] != 1) D("Admin")->commonExecute("agent", array("agent" => $param["agent"]), array("packageStatus" => 0), C("DB_PREFIX_API"));
                $this->ajaxReturn(array("Result" => true, "Msg" => "修改配置成功！"));
            } else {
                $this->ajaxReturn(array("Result" => false, "Msg" => "修改配置失败！"));
            }
        } else {
            $id = I("id");
            if (!$id) {
            ...
            $param = D("Admin")->commonQuery("adver_param", array("id" => $id), 0, 1, "*", C("DB_PREFIX_API"));
            if (!$param) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "获取不到参数配置"));
            }

            $agent  = D("Admin")->commonQuery("agent", array("agent" => $param["agent"]), 0, 1, "agentName", C("DB_PREFIX_API"));
            $advter = D("Admin")->commonQuery("advteruser", array("id" => $param["advteruser_id"]), 0, 1, "company_name");
            $num    = 0;
            for ($i = 1; $i <= 10; $i++) {
                if (!$param["param" . $i]) {
                    break;
            ...
            $this->assign("name", $agent["agentName"]);
            $this->assign("advter", $advter["company_name"]);
            $response = $this->fetch();
            $this->ajaxReturn(array("Result" => true, "Html" => $response));
        }
    }
    ...
    public function paramCheck()
    {
        ...
        if (empty($request)) {
            $this->ajaxReturn(array("Result" => false, "Msg" => "包体编号为空！"));
        }

        $map["status"] = 0;
        $map["agent"]  = $request;
        $agent         = D("Admin")->commonQuery("agent", $map, 0, 1, "agent,agentName,gameType,advteruser_id,packageStatus,pid", C("DB_PREFIX_API"));
        if (!$agent) {
            $this->ajaxReturn(array("Result" => false, "Msg" => "包体编号输入错误！"));
        }

        if ($agent["gameType"] == "1" && !isset($this->sdkApiAdvter[$agent["advteruser_id"]])) {
            $this->ajaxReturn(array("Result" => false, "Msg" => "该包体对应的广告商暂不开启配置！"));
        }
        ...
        $advter = getDataList("advteruser", "id");
        $param  = getDataListForKeys("adver_param", array("agent", "advteruser_id"), C("DB_PREFIX_API"), array("agent" => $request));
        if ($agent["gameType"] == "1") {
            if (isset($param[$request . "_" . $agent["advteruser_id"]])) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "该包体已配置广告商参数！"));
            }

            $arr = array(
                "id"    => $agent["advteruser_id"],
                "name"  => $advter[$agent["advteruser_id"]]["company_name"],
                "type"  => 1,
                "param" => $this->sdkApiAdvter[$agent["advteruser_id"]],
            );
            $res = array(
                "Result" => true,
            ...
            $this->ajaxReturn($res);
        } elseif ($agent["gameType"] == "2") {
            $i  = 0;
            $is = array();
            $no = array();
            foreach ($this->sdkApiAdvter as $key => $value) {
                $list = array(
                    "id"    => $key,
                    "name"  => $advter[$key]["company_name"],
                    "param" => $value,
                );
                if (isset($param[$request . "_" . $key])) {
                    $list["type"] = 0;
                    $no[]         = $list;
                } else {
                    $i            = 1;
            ...
            $arr = array_merge($is, $no);
            if (!$i) {
                $this->ajaxReturn(array("Result" => false, "Msg" => "该包体已配置全部广告商参数！"));
            }
        ...
        $this->ajaxReturn(array("Result" => false, "Msg" => "包体数据有错！"));
    }
    ...
    public function cdnList()
    {
        if (IS_POST) {
            ...
            $data['domain'] && $map['domain'] = array('like', '%' . $data['domain'] . '%');
            $res                              = D('Admin')->getBuiList('cdn', $map, $start, $pageSize);
            $results                          = $res['count'];
            foreach ($res['list'] as $key => $val) {
                if ($val['type'] == 1) {
                    $res['list'][$key]['type'] = '<span style="color:#00a3ff">腾讯云</span>';
                } else {
                    $res['list'][$key]['type'] = '阿里云';
                }

                if ($val['status']) {
                    $res['list'][$key]['status'] = '<span style="color:green">启用</span>';
                } else {
                    $res['list'][$key]['status'] = '<span style="color:red">关闭</span>';
                }
                $res['list'][$key]['opt'] = createBtn('<a href="javascript:;" onclick="cdnEdit(' . $val['id'] . ',this)">编辑</a>');
                $rows[]                   = $res['list'][$key];
            }
            $arr = array('rows' => $rows, 'results' => $results);
            exit(json_encode($arr));
