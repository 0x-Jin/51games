...
class Baidu2Model extends SdkModel
{
    ...
    public function __construct(){
        ...
        //验证地址
        $this->login_url    = "http://query.u.duoku.com/query/cploginstatequery";
        //充值回调地址
    ...
    public function loginCheck($data)
    {
        ...
        $param  = array(
            ...
            "Sign"          => md5($key["AppId"].$data["token"].$key["SecretKey"])
        );
        ...
        //验证成功
        if ($Result["ResultCode"] == "1" && $Result["Sign"] == md5($Result["AppID"].$Result["ResultCode"].$Result["Content"].$key["SecretKey"])) {
            //Content参数需要urldecode后再进行base64解码
            $result = json_decode(base64_decode(urldecode($Result["Content"])), true);
