...
class IcebirdModel extends SdkModel
{
    ...
    public function __construct()
    {
        ...
        //验证地址
        $this->login_url        = "https://oauth.ibingniao.com/oauth/token";
        //充值回调地址
        ...
        if (!$result || $Result["ret"] != "1") {
            ...
            $res = array(
                ...
                "Data"      => array(
                    ...
                    "channelUserName"   => $data["userName"]? $data["userName"]: $Result["content"]["user_id"],
                    "extInfo"           => array(
                        "token"         => $Result["content"]["access_token"],
                        "uid"           => $Result["content"]["user_id"]
                    ),
                )
            );
            return $res;
    ...
    {
        $data               = $_REQUEST;
        $data["sandbox"]    = $_SERVER["HTTP_X_BNPAY_SANDBOX"];
    ...
    public function callbackCheck($data)
    {
        ...
        //进行签名验证
        $str    = "app_key=".$data["app_key"]."&app_order_id=".$data["app_order_id"]."&app_role_id=".$data["app_role_id"]."&order_id=".$data["order_id"]."&pay_result=".$data["pay_result"]."&product_id=".$data["product_id"]."&server_id=".$data["server_id"]."&total_fee=".$data["total_fee"]."&user_id=".$data["user_id"] ."&jh_sign=".$key["AppSecret"]."&time=".$data["time"];
    ...
    {
        //返回失败
        echo json_encode(array("ret" => $num + 1));
        exit();
    }
}