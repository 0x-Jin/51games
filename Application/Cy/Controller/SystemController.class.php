...
class SystemController extends BackendController
{
    ...
    public function operationList()
    {
        if(IS_POST){
            $data = I();
            $map = array();
            $admin = getDataList('admin','real',C('DB_PREFIX'),array('real'=>$data['author']));
            ...
            $admin_list = getDataList('admin','name',C('DB_PREFIX'));
    ...
    public function menuEdit()
    {
        if(IS_POST){
            ...
            if(D('Admin')->commonExecute('menu',array('id'=>$data['id']),$data)){
            ...
            $id = I('id',0,'intval');
            $menu_info = D('Admin')->commonQuery('menu',array('id'=>$id));
    ...
    public function menuAdd()
    {
        if(IS_POST){
            ...
            if(D('Admin')->commonAdd('menu',$data)){
    ...
    public function menuDelete()
    {
        ...
        if(D('Admin')->commonDelete('menu',array('id'=>array('in',$id)))){
    ...
    public function user()
    {
        if(IS_POST){
            ...
            $department = array();
            foreach ($this->partment as $v) {
                $department[$v["partment_id"]] = $v["name"];
            }
            $list = D('Admin')->getUser($map,$start,$pageSize);
            $results = $list['count'];
            foreach ($list['list'] as $key=>$val){
                $list['list'][$key]['department'] = $department[$val["partment"]];
    ...
    public function userEdit()
    {
        if(IS_POST){
            ...
            if($data['manager_id'] != 16 && $data['manager_id'] != 4){
                //非CP账号清除game_id
                $data['game_id']   = 0;
                $data['user_type'] = 1; //我方账户
            }elseif($data['manager_id'] == 4){
                $data['user_type'] = 2; //广告商账户
            }elseif($data['manager_id'] == 16){
                if($data['game_id'][0] == 0) unset($data['game_id']);
                $data['game_id']   = !isset($data['game_id']) ? 0 : implode(',', $data['game_id']);
                $data['user_type'] = 3; //CP方账户
            }

            $data['principal_id'] = implode(',', $data['principal_id']);
            $data['backstage_account_id'] = !isset($data['backstage_account_id']) ? 0 : implode(',', $data['backstage_account_id']);
            $data['updateTime'] = time();
            if(D('Admin')->commonExecute('admin',array('id'=>$data['id']),$data)){
            ...
            $data = I();
            $manager = D('Admin')->getManager(array('status'=>0));
            $user_info = D('Admin')->commonQuery('admin',array('id'=>$data['id']),0,1,'*');
            ...
            $backstage  = M("advter_account")->alias("a")->join("LEFT JOIN la_advteruser b ON b.id = a.advteruserId")->field("a.id,CONCAT(IF(a.departmentId = 1, '一部', '二部' ), '-', b.company_name, '-',a.account, IF(a.status = 1, '', '【关闭】' )) AS backstage_name")->order("a.advteruserId ASC, a.departmentId ASC, a.id ASC")->select();
    ...
    public function userAdd()
    {
        if (IS_POST) {
            ...
            if(D('Admin')->commonQuery('admin',array('name'=>$data['name']))){
            ...
            $data['principal_id'] = !isset($data['principal_id']) ? 0 : implode(',', $data['principal_id']);
            $data['backstage_account_id'] = !isset($data['backstage_account_id']) ? 0 : implode(',', $data['backstage_account_id']);
            unset($data['repassword']);
            
            if (D('Admin')->commonAdd('admin', $data)) {
    ...
    public function role()
    {
        if(IS_POST){
            ...
            $list = D('Admin')->getManager($map,$start,$pageSize,'limit');
            $results = $list['count'];
            foreach ($list['list'] as $key=>$val){
    ...
    public function roleEdit()
    {
        if(IS_POST){
            ...
            $data['updateTime'] = time();
            if(D('Admin')->commonExecute('manager',array('id'=>$data['id']),$data)){
            ...
            $data = I();
            $role_info = D('Admin')->commonQuery('manager',array('id'=>$data['id']));
    ...
    public function clearCache()
    {
        if(IS_POST){
            ...
            $user = D('Admin')->commonQuery('admin',array('id'=>$data['id']));
            $cacheName = $user['name'].'_'.$user['manager_id'].'_'.$user['partment'].'_FusionAgentCache';
            if(S($cacheName,null)){
                IS_AJAX && $this->ajaxReturn(array(
                    'status'=>1,
                    'info'=>'清除成功'
    ...
    public function roleDelete()
    {
        ...
        is_int($id) && $id = array($id);
        if(D('Admin')->commonDelete('manager',array('id'=>array('in',$id)))){
    ...
    public function roleAdd()
    {
        if (IS_POST) {
            ...
            if (D('Admin')->commonAdd('manager', $data)) {
    ...
    public function auth()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            //清空权限
            D('Admin')->commonDelete('manager_menu',array('manager_id'=>$id));
            if (is_array($_POST['menu_id']) && count($_POST['menu_id']) > 0) {
                foreach ($_POST['menu_id'] as $menu_id) {
                    D('Admin')->commonAdd('manager_menu',array('manager_id'=>$id,'menu_id'=>$menu_id));
                }
            }
