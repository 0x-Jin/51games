...
class WebsiteController extends BackendController
{
    ...
    public function home()
    {
        if (IS_POST) {
            ...
            foreach ($res as $key => $val) {
                ...
                $res[$key]['opt']       = createBtn('<a href="javascript:;" onclick="homeEdit('.$val['id'].',this)">编辑</a> | <a href="'.U('Web/Index/',array('abbr'=>$val['abbr'])).'" target="_blank" onmouseover="showQR(\''.'http://fx.chuangyunet.net/'.$val['abbr'].'/wap'.'\',this)" onmouseout="hideQR()">预览</a>');
            }
    ...
    public function homeEdit()
    {
        if (IS_POST) {
            ...
            if ($res) {
                $str = "";
                foreach ($data as $k => $v) {
                    if ($v == $home[$k]) continue;
                    if ($k == "name") $str .= "，官网名称由“{$home[$k]}”改为“{$v}”";
                    if ($k == "abbr") $str .= "，名称缩写由“{$home[$k]}”改为“{$v}”";
                    if ($k == "agent") $str .= "，渠道号由“{$home[$k]}”改为“{$v}”";
                }
    ...
    public function article()
    {
        if (IS_POST) {
            ...
            $res        = D("Website")->commonQuery("article", $map, $start, $pageSize);
            foreach ($res as $key => $val) {
    ...
    public function articleAdd()
    {
        if (IS_POST) {
            ...
            $data["content"] = preg_replace('/\/static\/admin\/js\/ueditor\/php\/upload/','http://img.chuangyunet.net/static/admin/js/ueditor/php/upload',$data['content']);
    ...
    public function articleEdit()
    {
        if (IS_POST) {
            ...
            $data["content"] = preg_replace('/src="\/static\/admin\/js\/ueditor\/php\/upload/','src="http://img.chuangyunet.net/static/admin/js/ueditor/php/upload',$data['content']);
    ...
    public function slideList()
    {
        if (IS_POST) {
            ...
            if($this->departmentId){
                $agents     = getDataList('home','agent',C('DB_PREFIX_WEBSITE'),array('departmentId'=>$this->departmentId),'WEBSITE');
    ...
    public function copyWebSite($abbr,$tpl='ztfyl')
    {
        ...
        foreach ($webType as $webType) {
            ...
            //复制模板
            if(!file_exists(APP_PATH.'Admin/View/'.$webType.'/'.$abbr)) mkdir(APP_PATH.'Admin/View/'.$webType.'/'.$abbr,0755,true);
            ...
            if(count($file) > 0){
                ...
                if(!file_exists('./static/'.strtolower($webType).'/'.$abbr)) mkdir('./static/'.strtolower($webType).'/'.$abbr,0755,true);
                if(!file_exists('./static/'.strtolower($webType).'/'.$abbr.'/css')) mkdir('./static/'.strtolower($webType).'/'.$abbr.'/css',0755,true);
                if(!file_exists('./static/'.strtolower($webType).'/'.$abbr.'/images')) mkdir('./static/'.strtolower($webType).'/'.$abbr.'/images',0755,true);
                if(!file_exists('./static/'.strtolower($webType).'/'.$abbr.'/js')) mkdir('./static/'.strtolower($webType).'/'.$abbr.'/js',0755,true);

                $dirPath = scandir($staticDir);
                foreach ($dirPath as $dir) {//旧目录
                    if($dir == '.' || $dir == '..') continue;
                    $oldFile = scanfiles($staticDir.$dir);
                    
                    foreach($oldFile as $oldFileName){
                        if(strpos($oldFileName, '.html')) continue;
