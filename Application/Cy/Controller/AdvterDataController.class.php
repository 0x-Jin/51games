...
class AdvterDataController extends BackendController
{
    ...
    public function advData()
    {
        if(IS_POST){
            ...
            foreach ($res['list'] as $key=>$val){
                    //如果游戏汇总全是0则去除
                    // 'a.dayTime, a.agent, a.game_id,SUM(a.allPayUser) AS allPayUser, SUM(a.newDevice) AS newDevice, SUM(a.newUser) AS newUser, SUM(a.oldUserLogin) AS oldUserLogin, SUM(a.DAU) AS DAU, SUM(c.day1) AS day1, SUM(a.allPay) AS allPay,SUM(a.newPay) AS newPay,SUM(a.newPayUser) AS newPayUser,SUM(a.totalPay) AS totalPay, a.advteruser_id, IFNULL(SUM(b.cost),0) AS cost';
                    if(empty($val['allPayUser']) && empty($val['newDevice']) && empty($val['newUser']) && empty($val['oldUserLogin']) && empty($val['DAU']) && empty($val['day1']) && empty((float)$val['allPay']) && empty((float)$val['newPay']) && empty($val['newPayUser']) && empty((float)$val['totalPay']) && empty((float)$val['cost'])){
                        unset($res['list'][$key]);
                        // var_dump($res['list'][$key]);
                        continue;
                    }else{
                        foreach ($res['list'][$key]['children'] as $k => $v) {
                            if(empty($v['allPayUser']) && empty($v['newDevice']) && empty($v['newUser']) && empty($v['oldUserLogin']) && empty($v['DAU']) && empty($v['day1']) && empty((float)$v['allPay']) && empty((float)$v['newPay']) && empty($v['newPayUser']) && empty((float)$v['totalPay']) && empty((float)$v['cost'])){
                    ...
                    unset($sortArr);
                    // array_multisort($res['list'][$key]['children']);
                }
    ...
    private function principalInfo($data)
    {
        ...
        $arr = array();
        foreach($data as $k=>$v){
            if(!isset($agentInfo[$v['agent']])) continue;
    ...
    public function advCost()
    {
        if(IS_POST){
            ...
            $gameName  = M('advter_cost')->field('distinct trim(gameName) as gameName')->select();
...
//                date_default_timezone_set('Europe/London');
                ...
                foreach($res as $key => $val) {
                    ...
                    if ($max_num > $now_num) $num = $max_num - 1;
                ...
                foreach ($res as $key => $val) {
                    ...
                    if ($acc_sum) $account_str .= "<div class='br_line'></div><span class='br_span1'>汇总</span><span class='br_span2'>".$acc_sum."元</span>";
                    $arr            = explode("|", $val["payAmountTime"]);
                    $payAmountTime  = "";
                    $pay_sum        = 0;
                    $i              = 1;
                    ...
                    if ($pay_sum) $payAmountTime .= "<div class='br_line'></div><span class='br_span1'>汇总</span><span class='br_span2'>".$pay_sum."元</span>";
                    $amount_sum     += $val["amount"];
                    $pay_amount_sum += $pay_sum;
                    $invoice_sum    += $val["invoiceAmount"];
                    ...
                    foreach ($res[$key]["children"] as $k => $v) {
                        ...
                        if ($acc_sum) $account .= "<div class='br_line'></div><span class='br_span1'>汇总</span><span class='br_span2'>".$acc_sum."元</span>";
                        $arr            = explode("|", $v["payAmountTime"]);
                        $payAmountTime  = "";
                        $pay_sum        = 0;
                        if (!$arr) $arr[] = $v["payAmountTime"];
            ...
            foreach ($contractOption as $v) {
                if ($v !== "" && $v !== null) $option[$v] = $v;
            }
            if (!$option) $option = array('0' => '0');
            $this->assign("uid", $uid);
        ...
        if (IS_POST) {
            ...
            foreach ($data as $key => $val) {
                if (strpos($key, "accountA") !== false) {
                    ...
                    if ($val && $data["accountM".$account_id]) $account .= "|".$val.",".$data["accountM".$account_id];
                }
                if (strpos($key, "payAmountTimeA") !== false) {
                    $payAmountTime_id   = substr($key, 14);
                    if ($val && $data["payAmountTimeT".$payAmountTime_id]) $payAmountTime .= "|".$val.",".strtotime($data["payAmountTimeT".$payAmountTime_id]);
                }
            }
            ...
            if($res){
                ...
                foreach ($data as $k => $v) {
                    if ($k == "partment") $str .= "，部门为“".($v == 1? "发行1部": "发行2部")."”";
                    if ($k == "company") $str .= "，签约单位为“".$v."”";
                    if ($k == "contract") $str .= "，合同名称为“".$v."”";
                    if ($k == "contractNo") $str .= "，合同编号为“".$v."”";
                    if ($k == "game") $str .= "，游戏名称为“".$v."”";
                    if ($k == "type") $str .= "，合同类别为“".$v."”";
                    if ($k == "account" && $v) {
                        $account_arr    = explode("|", $v);
                        if (!$account_arr) $account_arr[] = $v;
                        foreach ($account_arr as $list) {
                            $list_arr   = explode(",", $list);
                            $str        .= "，充值账号“".$list_arr[0]."”，充值金额“".($list_arr[1]? $list_arr[1]: "0")."”";
                        }
                    }
                    if ($k == "info") $str .= "，主要条款为“".$v."”";
                    if ($k == "status") $str .= "，效果为“".$status[$v]."”";
                    if ($k == "payType") $str .= "，结算方式为“".$payType[$v]."”";
                    if ($k == "fileTime" && $v) $str .= "，签订日期为“".date("Y-m-d", $v)."”";
                    if ($k == "startTime" && $v) $str .= "，生效日期为“".date("Y-m-d", $v)."”";
                    if ($k == "endTime" && $v) $str .= "，终止日期为“".date("Y-m-d", $v)."”";
                    if ($k == "principalId") {
                        if ($v) {
                    ...
                    if ($k == "amount") $str .= "，总金额为“".$v."”";
                    if ($k == "payAmountTime" && $v) {
                        $amountTime_arr    = explode("|", $v);
                        if (!$amountTime_arr) $amountTime_arr[] = $v;
                        foreach ($amountTime_arr as $list) {
                            $list_arr   = explode(",", $list);
                            $str        .= "，支付金额“".$list_arr[0]."”，支付时间“".($list_arr[1]? date("Y-m-d", $list_arr[1]): "")."”";
                        }
                    }
                    if ($k == "receipt" && $v) $str .= "，票据号为“".$v."”";
                    if ($k == "invoiceAmount") $str .= "，收到发票金额为“".$v."”";
                    if ($k == "ext" && $v) $str .= "，备注为“".$v."”";
                }
    ...
    public function contractEdit()
    {
        if (IS_POST) {
            ...
            if($res !== false){
                ...
                foreach ($data as $k => $v) {
                    if ($v == $contract[$k]) continue;
                    if ($k == "partment") $str .= "，部门由“".($contract[$k] == 1? "发行1部": "发行2部")."”改为“".($v == 1? "发行1部": "发行2部")."”";
                    if ($k == "company") $str .= "，签约单位由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "contract") $str .= "，合同名称由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "contractNo") $str .= "，合同编号由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "childNo") $str .= "，信息/签或编号由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "game") $str .= "，游戏名称由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "type") $str .= "，合同类别由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "account" && $v) {
                        $old_str        = "";
                        $new_str        = "";
                        $account_arr    = explode("|", $contract[$k]);
                        if (!$account_arr) $account_arr[] = $contract[$k];
                        foreach ($account_arr as $list) {
                            $list_arr   = explode(",", $list);
                    ...
                    if ($k == "info") $str .= "，主要条款由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "status") $str .= "，效果由“".$status[$contract[$k]]."”改为“".$status[$v]."”";
                    if ($k == "payType") $str .= "，结算方式由“".$payType[$contract[$k]]."”改为“".$payType[$v]."”";
                    if ($k == "fileTime" && $v) $str .= "，签订日期由“".date("Y-m-d", $contract[$k])."”改为“".date("Y-m-d", $v)."”";
                    if ($k == "startTime" && $v) $str .= "，生效日期由“".date("Y-m-d", $contract[$k])."”改为“".date("Y-m-d", $v)."”";
                    if ($k == "endTime" && $v) $str .= "，终止日期由“".date("Y-m-d", $contract[$k])."”改为“".date("Y-m-d", $v)."”";
                    if ($k == "principalId") {
                        if ($v) {
                            $principal      = D("Admin")->commonQuery("principal", array("id" => $v), 0, 1, "principal_name");
                    ...
                    if ($k == "amount") $str .= "，总金额由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "payAmountTime" && $v) {
                        $old_str        = "";
                        $new_str        = "";
                        $amountTime_arr = explode("|", $contract[$k]);
                        if (!$amountTime_arr) $amountTime_arr[] = $contract[$k];
                        foreach ($amountTime_arr as $list) {
                            $list_arr   = explode(",", $list);
                    ...
                    if ($k == "receipt" && $v) $str .= "，票据号由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "invoiceAmount") $str .= "，收到发票金额由“".$contract[$k]."”改为“".$v."”";
                    if ($k == "ext" && $v) $str .= "，备注由“".$contract[$k]."”改为“".$v."”";
                }
...
//                $this->error("合同编辑失败！");
            ...
            if ($info["account"]) {
                ...
                if (!$arr) $arr[] = $info["account"];
                foreach ($arr as $list) {
                    $list_arr   = explode(",", $list);
                    if ($list_arr[0]) $account[] = array("accountA" => $list_arr[0], "accountM" => $list_arr[1]);
                }
            }
            if ($info["payAmountTime"]) {
                $arr    = explode("|", $info["payAmountTime"]);
                if (!$arr) $arr[] = $info["payAmountTime"];
                foreach ($arr as $list) {
                    $list_arr   = explode(",", $list);
                    if ($list_arr[0]) $payAmountTime[] = array("payAmountTimeA" => $list_arr[0], "payAmountTimeT" => ($list_arr[1]? date("Y-m-d", $list_arr[1]): ""));
                }
            }
    ...
    public function financeCost()
    {
        if (IS_POST) {
            $data           = I();
            $map["date"]    = $data["date"];
            if ($data["companyName"]) $map["companyName"] = $data["companyName"];
            if ($data["account"]) $map["account"] = array("LIKE", "%".$data["account"]."%");
            //财务可以看全部数据，其他人只看自己
...
//                date_default_timezone_set("Europe/London");
                ...
                if ($res) {
                    foreach ($res["list"] as $v) {
                        $list                   = $v;
                        $list["rebateName"]     = $v["rebate"]."%";
                        $list["rebateTypeName"] = $v["rebateType"]? "返现": "返点";
                        $list["costName"]       = "<span id='cost_".$v["id"]."'>".$v["cost"]."</span>";
                        $list["realCostName"]   = "<span id='realCost_".$v["id"]."'>".$v["realCost"]."</span>";
                        if ($v["twoExamine"]) {
                ...
                $arr = array("rows" => $rows, "results" => $res? $res["count"]: 0);
                exit(json_encode($arr));
            }
        } else {
    ...
    public function financeCostEdit()
    {
        if (IS_POST) {
            $data   = I("");
            if (!$data["id"] || !$data["cost"] || !$data["realCost"]) $this->ajaxReturn(array("Result" => false, "Msg" => "数据错误！"));
            $data["oneExamineTime"] = 0;
            $res    = D("admin")->commonExecute("finance_cost", array("id" => $data["id"]), $data);
            if (!$res)  $this->ajaxReturn(array("Result" => false, "Msg" => "编辑失败！"));
    ...
    public function financeCostDelete()
    {
        ...
        if (!$res) $this->ajaxReturn(array("Result" => false, "Msg" => "删除失败！"));
    ...
    public function examineCost()
    {
        ...
        if (!$info) $this->ajaxReturn(array("Result" => false, "Msg" => "无此消耗信息！"));
        if ($info["twoExamine"]) $this->ajaxReturn(array("Result" => false, "Msg" => "该消耗已被财务审核，无法再次审核！"));
        $time   = time();
    ...
    public function _before_update($data)
    {
        if($this->table == 'advter_cost'){
            $agent      = getDataList('agent','agent',C('DB_PREFIX_API'));
            if(!isset($agent[$data['agent']])) $this->error('渠道号有误');

            $advterUser = getDataList('advteruser','company_name',C('DB_PREFIX'));
            if(!isset($advterUser[$data['media']])) $this->error('媒体有误');
    ...
    public function payRemain()
    {
        if(IS_POST){
            ...
            if($data['export'] == 1){
                ...
                for($i=0;$i<=120;$i++){
                    if(in_array($i, $day_arr)){
                        $i==1 && $col['day'.$i] = '次日付费留存率';
                        $col['day'.$i] = ($i).'日付费留存率';
                    }
                }
                array_unshift($rows, $col);
                export_to_csv($rows,'付费衰减',$col);
                exit();
            }
            $arr = array('rows'=>$rows ? $rows : array(), 'results'=>$results, 'pageSummary'=>$pagesummary);
            unset($rows,$remainArr);
