...
class AdvClickLogIos extends Connect
{
    const DS           = DIRECTORY_SEPARATOR;
    private $sql       = ''; //sql语句
    private $limit     = 5000; //限定5000条
    private $num       = 0; //计数变量
    ...
    public function __construct()
    {
        ...
        $this->log_start =  "\r\n" ."【AdvClickIosStart:".date('Y-m-d H:i:s')."】";
        $this->eventRecord($this->log_start, 'ios', 'AdvClickIosRun.log', false, 2, false);

        $res = $this->advterClick();

        $this->log_end = "\r\n" . "【AdvClickIosEnd:".date('Y-m-d H:i:s')."】";
        $this->eventRecord($this->log_end, 'ios', 'AdvClickIosRun.log', false, 2, false);
    ...
    private function advterClick()
    {
        ...
        foreach ($data as $k => $v) {
            $sql = "insert into lgame.`la_ios_click_log`(advter_id,game_id,agent,muid,adUserId,advterType,ip,mac,clickTime,createTime) values('{$v['advter_id']}','{$v['game_id']}','{$v['agent']}','{$v['muid']}','{$v['adUserId']}','{$v['advterType']}','{$v['ip']}','{$v['mac']}','{$v['clickTime']}','{$v['createTime']}');";
            ...
            if ($this->num >= $this->limit) {
                ...
                if (false !== mysqli_multi_query($this->conn, $this->sql)) {
                    ...
                    error_log("\r\n" . '[ ' . date('Ymd H:i:s') . mysqli_error($this->conn) . '] ', 3, __DIR__ . '/AdvClickLog' . self::DS . 'AdvClickLogIos.log');
                    continue;
                }
            }
    ...
    private function mongoData($db, $table)
    {
        ...
        //是否存在前一次的最大时间
        $filePath = dirname(__FILE__) . "/AdvClickLog/ios/maxTime.log";
        $map      = array();
        ...
        if ($data) {
            ...
            //记录最大时间
            $this->eventRecord($this->maxTime, 'ios', 'maxTime.log');
