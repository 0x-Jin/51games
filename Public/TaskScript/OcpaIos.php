...
class OcpaIos extends Connect
{
	...
	public function __construct()
	{
		...
		//判断ocpa锁是否未开启
        $this->filename = __DIR__.'/OcpaLog/ios/1ocpaLock.log';
	...
	private function mongoData($db, $table)
	{
	    ...
	    //是否存在前一次的最大时间
	    $filePath = dirname(__FILE__) . "/OcpaLog/ios/1maxTime.log";
	    $map      = array();

	    $config = $this->mongo->select('ocpa',array('status'=>'0','departmentId'=>'1'));
	    ...
	    if ($data) {
	        ...
	        //记录最大时间
	        $this->eventRecord($this->maxTime, 'ios', '1maxTime.log');
