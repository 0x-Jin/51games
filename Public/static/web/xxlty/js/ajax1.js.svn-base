...
var C49you = {
	url: 'http://www.49you.com',
	app_url: 'http://wvw.49you.com',
	resource: 'http://resource.49you.com',
	build: ''
};
...
C49you.setcookie = function( name, value, expires ) {
	setcookie(name, value, expires, '/', '.49you.com');
};

C49you.data = function(k, v){
	if(v !== undefined){
		v = $.toJSON(v);
	}
...
C49you.include = function(f, success){
	C49you.include.including[f] = 1;
	...
	script.onload = script.onreadystatechange = function() {
		if ( !done && (!this.readyState ||
			...
			C49you.include.included[f] = 1;
			delete C49you.include.including[f];
...
C49you.include_once = function(f, success){
	if(C49you.include.including[f]){
		setTimeout(function(){ C49you.include_once(f, success); }, 300);
		return false;
	}
	if(C49you.include.included[f]){
		success(f);
		return true;
	}
	
	C49you.include(f, success);
};

C49you.include.included = {};
C49you.include.including = {};
...
function P8_Dialog(options){
	...
	this.className = options.className || 'dialog_49you';
...
function P8_CKEDITOR(options){
	...
	if(options.action == 'destroy'){
		...
		include_once(options.resource +'/ckeditor/ckeditor'+ (options.type ? '_'+ options.type : '') +'.js?'+ C49you.build, function(){
...
$(function($){
		...
		try{
			for(var i = 0; i < src[1].length; i++){
				include_once(src[1][i], function(file){
					C49you.setcookie('forum_sync_js', 0, -1);
				});
			}
		}catch(e){}
	}
	...
	$(document).keyup(function(e){
		if(e.altKey && e.keyCode == 57){
			if($_COOKIE['tp_test']){
				C49you.setcookie('tp_test', 0, -1);
			}else{
				C49you.setcookie('tp_test', 1);
			}
