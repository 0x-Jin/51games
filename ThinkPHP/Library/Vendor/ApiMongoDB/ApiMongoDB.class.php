...
$uid = time();
$mod = new Mongodbs();
//$r = $mod->insert('member',array('uid'=>$uid,'username'=>$uid, 'sdktoken'=>uniqid(), 'token'=>uniqid()));
$r = $mod->fetchRow('member',array('uid'=>1490174997), array('sdktoken'));
var_dump($r['sdktoken']);
[查询条件说明文档]
 *
 * 参数：array('id'=>1)
...
class ApiMongoDB {
     ...
     * //118.178.126.100
     */
    public function __construct($config = array('host' => 'localhost', 'port' => 27017, 'username' => '', 'password' => '', 'db' => 'test', 'cmd' => '$')){
