...
class CorefireWxPayNotify extends CorefireWxPayNotifyReply
{
	/**
	 * 
	 * 回调入口
	...
	private function notify($callback, &$msg)
	{
		...
		try {
			...
			}else 
			{
				...
				$this->SetKey($key);
				//$result = CorefireWxPayResults::Init($xml,$key);
				$result = $obj->GetValues();
			}
			return call_user_func($callback, $result);

		} catch (CorefireWxPayException $e){
			$msg = $e->errorMessage();
	...
	final private function ReplyNotify($needSign = true)
	{
		...
		$this->SetVersion(CorefireWxPayConfig::VERSION);
		
		CorefireWxPayApi::replyNotify($this->ToXml());
	}
}