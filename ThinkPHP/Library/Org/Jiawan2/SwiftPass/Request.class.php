...
require_once(dirname(__FILE__)."/PayHttpClient.class.php");
...
Class jw_Request{
    ...
    public function Request(){
        $this->resHandler = new \jw_ClientResponseHandler();
        $this->reqHandler = new \jw_RequestHandler();
        $this->pay = new \jw_PayHttpClient();
        $this->cfg = new \jw_Config();
...
//        $type == 2? $this->reqHandler->setParameter('appid',$this->cfg->C('appid')): null;
        ...
        $data = \jw_Utils::toXml($this->reqHandler->getAllParameters());
    ...
    public function submitRefund(){
        ...
        $this->reqHandler->createSign();//创建签名
        $data = \jw_Utils::toXml($this->reqHandler->getAllParameters());//将提交参数转为xml，目前接口参数也只支持XML方式
