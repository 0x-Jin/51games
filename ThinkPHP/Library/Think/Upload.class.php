...
class Upload {
     ...
     * 生成有序的名称
     */
    private function patch_savenames($extension, $typename='bg'){
        global $tk_name;
        if($typename === 'screenshot' || $typename === 'bg_file'){
            $this->__set('replace', true);
            return $this->__get('pathId') . '_' . $typename . '.' . $extension;
    
        }else{
            $this->__set('replace', false);
            empty($tk_name[$typename]) && $tk_name[$typename] = 0;
            if($typename === 'banner_file'){
                $tname = $this->__get('pathId') .'_banner'. $tk_name[$typename] . '.'.$extension;
            }elseif($typename === 'bg_file'){
                return $tname = $this->__get('pathId') .'_bg.'.$extension;
            }elseif($typename === 'btn_file'){
                $tname = $this->__get('pathId') .'_btn'. $tk_name[$typename] . '.'.$extension;
            }else{
                $tname = $this->__get('pathId') .'_'. $tk_name[$typename] . '.'.$extension;
            }

            if(file_exists($this->rootPath.$this->__get('savePath').DS.$tname)){
                $tk_name[$typename]++;
                return $this->patch_savenames($extension, $typename);
            }else{
                return $tname;
            }
        }
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            /* 生成保存文件名 */
            if($this->__get('pathId')){
                $file['savename']  =   $this->patch_savenames($file['ext'], $file['key']);
            }else{
    ...
    private function getSaveName($file) {
        ...
        return ($this->__get("encoding")? iconv("UTF-8", "GBK", $savename): $savename) . '.' . $ext;
    }
